...
let ultimaUbicacion = { lat: 19.4326, lng: -99.1332 }; // Ubicación por defecto

// 1. Endpoint que recibe las coordenadas desde Traccar Client
app.all('/api/posicion', (req, res) => {
  ...
  if (lat && lon) {
    ...
    console.log(`Nueva ubicación: ${lat}, ${lon}`);
  }
  res.status(200).send("OK");
});

// 2. Endpoint que entrega la última ubicación al mapa
app.get('/api/ubicacion-actual', (req, res) => {
  res.json(ultimaUbicacion);
});

// 3. Página web con el mapa interactivo
app.get('/', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html>
